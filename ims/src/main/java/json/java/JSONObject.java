...
import json.java.internal.Parser;
 ...
 */
public class JSONObject extends HashMap<Object, Object>  implements JSONArtifact
{
     ...
     */
    public static boolean isValidType(Class<? extends Object> clazz)
    {
     ...
     * @param writer - writer the serializer is writing to
     * @param serializer - serializer used to serialize the JSONObject
     * @return
     * @throws IOException
     */
    public String serialize(Writer writer, Serializer serializer) throws IOException {
        serializer.writeObject(this).flush();
        return writer.toString();
    }
    
    /**
     * Convert this object into a String of JSON text, sanitizing any undesirable values.
     * @param verbose
     * @return
     * @throws IOException
     */
    public String sanitizedSerialize(boolean verbose) {
        Serializer serializer;
    	StringWriter writer = new StringWriter();
    	
        if (verbose)
        {
            serializer = new SanitizeSerializerVerbose(writer);
        }
        else
        {
            serializer = new SanitizeSerializer(writer);
        }
    ...
    public Object put(Object key, Object value)
    {
        ...
        if (!isValidObject(value)) {
        	...
        	// If we have an embedded hashmap then convert it to another JSONObject and add it
        	if (value instanceof HashMap) {
        		JSONObject jsonNode = new JSONObject();
		      	jsonNode.putAll((HashMap<?, ?>)value);

		      	return super.put(key, jsonNode);
        	} else {
        		// Error
            	if (value != null) {
